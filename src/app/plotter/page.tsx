...
// ใช้ dynamic import เพื่อแก้ปัญหา "self is not defined" (ปลาขาดน้ำ)
const Plot = dynamic<PlotParams>(() => import("react-plotly.js"), {
  ssr: false,
});

// สร้าง Interface สำหรับ plotLayout ที่ยืดหยุ่น
...
export default function PlotterPage() {
  ...
  // Function to reset all fields and the plot
  const resetFields = () => {
    setExpression2d("");
    setXMin2d("");
    setXMax2d("");
  ...
  const handlePlot2D = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/plotter/2d?${params.toString()}`);
  ...
  const handlePlotImplicit = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8081/api/plotter/implicit?${params.toString()}`);
  ...
  const handlePlot3D = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8081/api/plotter/3d?${params.toString()}`);
  ...
  const handlePlotComplex = async () => {
    ...
    try {
      ...
      const url = `http://localhost:8081/api/plotter/complex?${params.toString()}`;
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.tabsContainer}>
        <button onClick={() => setPlotType("2d")} className={`${styles.tabButton} ${plotType === "2d" ? styles.tabActive : ""}`}>
          2D Explicit
        </button>
        <button onClick={() => setPlotType("implicit")} className={`${styles.tabButton} ${plotType === "implicit" ? styles.tabActive : ""}`}>
          2D Implicit
        </button>
        <button onClick={() => setPlotType("3d")} className={`${styles.tabButton} ${plotType === "3d" ? styles.tabActive : ""}`}>
          3D
        </button>
        <button onClick={() => setPlotType("complex")} className={`${styles.tabButton} ${plotType === "complex" ? styles.tabActive : ""}`}>
          Complex
        </button>
        <button onClick={resetFields} className={styles.resetButton}>
          Reset
        </button>
      </div>
      ...
      {plotType === "complex" && (
        <div className={styles.complexInputContainer}>
          <p className={styles.complexNote}>
            Enter the real part u(x,y) and imaginary part v(x,y) of f(z).<br />
